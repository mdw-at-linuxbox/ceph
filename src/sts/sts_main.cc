...
static int process_request(STSStore *const store,
	RGWREST *const rest,
	STSRequest *const req,
...
//  s->trans_id = store->unique_trans_id(req->id);
  ...
  RGWHandler_REST *handler = rest->get_handler((RGWRados*)store, s,
	frontend_prefix, client_io, &mgr, &init_error);
  if (init_error != 0) {
...
{
  STSFCGXRequest *const req = static_cast<STSFCGXRequest *>(r);
  ...
  int ret = process_request(store, rest, req, uri_prefix, &client_io);
  if (ret < 0) {
...
class STSAsioFrontend : public STSFrontend {
  STSFrontendConfig *conf;
...
public:
  STSAsioFrontend(STSProcessEnv& pe) : conf(_conf), ctx(NULL) {
  }
...
int main(int argc, const char **argv)
{
  ...
  get_str_list(g_conf->sts_enable_apis, apis);
...
*/
  ...
  get_str_list(g_conf->sts_frontends, ",", frontends);
  ...
  for (multimap<string, STSFrontendConfig *>::iterator fiter = fe_map.begin(); fiter != fe_map.end(); ++fiter) {
    ...
    if (framework == "fastcgi" || framework == "fcgi") {
      ...
      fe = new STSAsioFrontend(env);
#endif
    } else if (framework == "civetweb" || framework == "mongoose") {
      STSProcessEnv env = { store, &rest };
